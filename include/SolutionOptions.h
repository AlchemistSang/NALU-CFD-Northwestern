...
public:
  ...
  // Variables for energy conservation equation
  bool includeLatent_;
  bool balancedForce_;
  bool projectDarcy_;
  bool redistCorrection_;
  bool addRecoil_;
  double liquidus_;
  double solidus_;
  double latent_;
  double emissivity_;
  double ambientTemp_;
  double keyholeDepth_;
  double keyholeRadius_;
  double substrateLevel_;
  double toolXYZ_[3];

  double sigma0_;
  double dsigdt_;
  double darcyBig_;     // Large, impede flow in solid phase
  double darcySmall_;  // Small, avoid division by zero
  double vofEps_;      // diffusion parameter during CLS compression
  double redistPenalty_;      // penalty parameter during redistancing
  int numNonLinearVolCorrectionIter_;
  ...
  double beamPower_;
  double beamEff_;
  double beamVelocity_; //MJ: laser beam velocity
  double powderDepth_; //MJ: laser beam velocity

  //MJ: prandtl number for computing turbulent conductivity
  double prandtl_;

  // variables for evaporation model
  double kB_, molecularWeight_, ambientP_, evapTemp_,
         latentEvap_, gasConstant_, molarMass_, meltLength_;
