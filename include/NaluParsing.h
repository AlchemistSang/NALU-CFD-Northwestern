...
//MJ: for every EqSys define a struct
// this is for epsilon from k-epsilon model
struct TurbKinDissRate {
  double TurbKinDissRate_;
  TurbKinDissRate()
    : TurbKinDissRate_(0.0)
  {}
};
...
struct WallUserData : public UserData {
  ...
  Velocity dx_;
  TurbKinEnergy tke_;
  TurbKinDissRate TKEdr_;   //MJ: BC for epsilon in k-epsilon model
  MixtureFraction mixFrac_;
  MassFraction massFraction_;
  LevelSet levelSet_;
  VolumeFraction volume_fraction_;
  ...
  bool isAdiabatic_;
  bool heatFluxSpec_;
  bool laserHeatFluxSpec_; 
  bool marangoniSpec_;
  bool evapHeatSpec_;
  bool evapContinuitySpec_;
  bool evapMomentumSpec_;
  bool isInterface_;
  bool refTempSpec_;
  bool htcSpec_;
  ...
  bool freeStreamDirichlet_;  //MJ: setting a Dirichlet BC for k and epsiln
                              // on the top of the melt pool
  ...
  WallUserData()
    : UserData(),
      isAdiabatic_(false),
      heatFluxSpec_(false),
      laserHeatFluxSpec_(false),
      freeStreamDirichlet_(false),
      marangoniSpec_(false),
      evapHeatSpec_(false),
      evapMomentumSpec_(false),
      evapContinuitySpec_(false),
      isInterface_(false),
      refTempSpec_(false),
      htcSpec_(false),
...
struct InflowUserData : public UserData {
  ...
  bool levelSetSpec_;
  bool volume_fractionSpec_;
  bool intensitySpec_;
  
  InflowUserData()
    : UserData(),
      uSpec_(false), tkeSpec_(false), sdrSpec_(false), TKEdrSpec_(false), mixFracSpec_(false), massFractionSpec_(false), levelSetSpec_(false), volume_fractionSpec_(false), intensitySpec_(false)
  {}
};
...
struct OpenUserData : public UserData {
  ...
  OpenUserData()
    : UserData(),
      uSpec_(false), pSpec_(false), tkeSpec_(false), sdrSpec_(false), TKEdrSpec_(false), mixFracSpec_(false),
      massFractionSpec_(false), levelSetSpec_(false), volume_fractionSpec_(false), intensitySpec_(false)
  {}
};
