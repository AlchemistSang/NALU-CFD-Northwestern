...
// Class Definition
//==========================================================================
// ComputeNCMdotDarcyElemAlgorithm - interior mdor for elem continuity
//==========================================================================
//--------------------------------------------------------------------------
//-------- constructor -----------------------------------------------------
//--------------------------------------------------------------------------
ComputeNCMdotDarcyElemAlgorithm::ComputeNCMdotDarcyElemAlgorithm(
  Realm &realm,
  stk::mesh::Part *part,
...
//--------------------------------------------------------------------------
void
ComputeNCMdotDarcyElemAlgorithm::execute()
{
  ...
  //MJ: get laser speed, whih is in X direction only
  const double scanSpeed = 0.0; //realm_.solutionOptions_->beamVelocity_;
      ...
      for ( int ip = 0; ip < numScsIp; ++ip ) {
        ...
        for ( int ic = 0; ic < nodesPerElement; ++ic ) {
          ...
          for ( int j = 0; j < nDim; ++j ) {
            ...
            p_invrhoGpdx[j] += r* ( p_Gpdx[nDim*ic+j]) * ( 1.0 / (nodalRho + projTimeScale * p_permeability[ic] ) );
        ...
        // MJ: add the contribution by beam velocity to the mass flow rate. It's important for advection
        // beam velocity has only x component
        p_uIp[0] += scanSpeed;
        ...
        for ( int j = 0; j < nDim; ++j ) {
          tmdot += (p_uIp[j] 
                    - projTimeScale*( invPermIp*p_dpdxIp[j] - p_invrhoGpdx[j] ) )*p_scs_areav[ip*nDim+j];
//          tmdot += (p_uIp[j] 
...
//--------------------------------------------------------------------------
void
ComputeNCMdotDarcyElemAlgorithm::assemble_edge_mdot()
{
