  ...
  const YAML::Node *y_equation_system = expect_map(y_node,"equation_systems");
  {
    ...
    const YAML::Node *y_systems = expect_sequence(*y_equation_system, "systems");
    {
      for ( size_t isystem = 0; isystem < y_systems->size(); ++isystem )
      {
        ...
        // MJ: added k-epsilon turbulence model
        else if( (y_eqsys = expect_map(y_system, "KEpsilonTransport", true)) ) {
          if (root()->debug()) NaluEnv::self().naluOutputP0() << "eqSys = tke/epsilon " << std::endl;
          eqSys = new KEpsilonTransportEquationSystem(*this);
        }
        ...
        else if( (y_eqsys = expect_map(y_system, "NonConsMixtureFraction", true)) ) {
          if (root()->debug()) NaluEnv::self().naluOutputP0() << "eqSys = mixFrac " << std::endl;
          bool ouputClipDiag = false;
          get_if_present_no_default(*y_eqsys, "output_clipping_diagnostic", ouputClipDiag);
          double deltaZClip = 0.0;
          get_if_present_no_default(*y_eqsys, "clipping_delta", deltaZClip);
          eqSys = new NonConsMixtureFractionEquationSystem(*this, ouputClipDiag, deltaZClip);
        }
        ...
        else if( (y_eqsys = expect_map(y_system, "NonConsEnthalpy", true)) ) {
          if (root()->debug()) NaluEnv::self().naluOutputP0() << "eqSys = enthalpy " << std::endl;
          double minT = 250.0;
          double maxT = 3000.0;
          get_if_present_no_default(*y_eqsys, "minimum_temperature", minT);
          ...
          bool ouputClipDiag = true;
          get_if_present_no_default(*y_eqsys, "output_clipping_diagnostic", ouputClipDiag);
          eqSys = new NonConsEnthalpyEquationSystem(*this, minT, maxT, ouputClipDiag);
        }
        ...
        else if( (y_eqsys = expect_map(y_system, "LevelSet", true)) ) {
          if (root()->debug()) NaluEnv::self().naluOutputP0() << "eqSys = MeshDisplacement " << std::endl;
          bool managePNG = false;
          get_if_present_no_default(*y_eqsys, "manage_png", managePNG);
          eqSys = new LevelSetEquationSystem(*this, managePNG);
        }          
        else if( (y_eqsys = expect_map(y_system, "Heaviside", true)) ) {
          if (root()->debug()) NaluEnv::self().naluOutputP0() << "eqSys = MeshDisplacement " << std::endl;
          bool managePNG = false;
          get_if_present_no_default(*y_eqsys, "manage_png", managePNG);
          eqSys = new HeavisideEquationSystem(*this, managePNG);
        }
        else if( (y_eqsys = expect_map(y_system, "Temperature", true)) ) {
          if (root()->debug()) NaluEnv::self().naluOutputP0() << "eqSys = Temperature " << std::endl;
          bool managePNG = false;
          get_if_present_no_default(*y_eqsys, "manage_png", managePNG);
          eqSys = new TemperatureEquationSystem(*this);
        }          
        else if( (y_eqsys = expect_map(y_system, "Intensity", true)) ) {
          if (root()->debug()) NaluEnv::self().naluOutputP0() << "eqSys = MeshDisplacement " << std::endl;
          bool managePNG = false;
          get_if_present_no_default(*y_eqsys, "manage_png", managePNG);
          eqSys = new IntensityEquationSystem(*this, managePNG);
        }
        else if( (y_eqsys = expect_map(y_system, "NonConsLowMachEOM", true)) ) {
          if (root()->debug()) NaluEnv::self().naluOutputP0() << "eqSys = NonConsLowMachEOM " << std::endl;
          bool elemCont = (realm_.realmUsesEdges_) ? false : true;
          get_if_present_no_default(*y_eqsys, "element_continuity_eqs", elemCont);
          eqSys = new NonConsLowMachEquationSystem(*this, elemCont);
        }          
        else if( (y_eqsys = expect_map(y_system, "NonConsLowEnergy", true)) ) {
          if (root()->debug()) NaluEnv::self().naluOutputP0() << "eqSys = NonConsEnergy " << std::endl;
          bool elemCont = (realm_.realmUsesEdges_) ? false : true;
          double minT = 250.0;
          double maxT = 3000.0;
          get_if_present_no_default(*y_eqsys, "minimum_temperature", minT);
          ...
          bool ouputClipDiag = true;
          get_if_present_no_default(*y_eqsys, "output_clipping_diagnostic", ouputClipDiag);
          eqSys = new NonConsEnergyEquationSystem(*this, minT, maxT, ouputClipDiag);
        }          
        else if( (y_eqsys = expect_map(y_system, "LSReinit", true)) ) {
          if (root()->debug()) NaluEnv::self().naluOutputP0() << "eqSys = LSReinit " << std::endl;
          bool managePNG = false;
          get_if_present_no_default(*y_eqsys, "manage_png", managePNG);
          eqSys = new LSReinitEquationSystem(*this, managePNG);
        }          
        else {
