...
public:
  nodalGradientElem(stk::mesh::Bucket & b, MasterElement & meSCS,
      ...
      ScalarFieldType & scalarQ, ScalarFieldType & scalarDiv,
  ...
  void operator()(stk::mesh::Bucket::size_type elem_offset){
    ...
    for ( int ip = 0; ip < numScsIp_; ++ip ) {
      ...
      for ( int ic = 0; ic < nodesPerElement_; ++ic ) {
        ...
        for ( int j = 0; j < nDim_; ++j ) {
           //lhsfacDiff += p_dndx[offSetDnDx+j]*p_scs_areav[ip*nDim_+j];
           nphiIp[j] += p_dndx[offSetDnDx+j]*p_scalarQ[ic];
        }
      ...
      for (int j = 0; j < nDim_; ++j)
      {
        qDiff += p_scs_areav[ip*nDim_+j] * nphiIp[j] / normphiIp;
      }
...
// Class Definition
//==========================================================================
// AssembleNodalDivGradElemAlgorithm - Green-Gauss gradient
//==========================================================================
//--------------------------------------------------------------------------
//-------- constructor -----------------------------------------------------
//--------------------------------------------------------------------------
AssembleNodalDivGradElemAlgorithm::AssembleNodalDivGradElemAlgorithm(
  Realm &realm,
  stk::mesh::Part *part,
...
//--------------------------------------------------------------------------
void
AssembleNodalDivGradElemAlgorithm::execute()
{
  stk::mesh::MetaData & meta_data = realm_.meta_data();
  ...
  for ( stk::mesh::BucketVector::const_iterator ib = elem_buckets.begin();
    ...
    nodalGradientElem nodeGradFunctor(b, *meSCS, p_shape_function, *scalarQ_, *scalarDiv_, *dualNodalVolume_, *coordinates_, nDim);
