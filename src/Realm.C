...
#include <property_evaluator/TempDependentPropAlgorithm.h>
...
//--------------------------------------------------------------------------
  ...
  : realms_(realms),
    ...
    BlockDataBasePostProcessing_(NULL),
...
Realm::~Realm()
{
  ...
  if ( NULL != BlockDataBasePostProcessing_ )
    delete BlockDataBasePostProcessing_;
...
Realm::look_ahead_and_creation(const YAML::Node & node)
{
  ...
  // look for nodal_data_base
  std::vector<const YAML::Node *> foundNodalDataQuery;
  NaluParsingHelper::find_nodes_given_key("nodal_data_base", node, foundNodalDataQuery);
  if ( foundNodalDataQuery.size() > 0 ) {
    if ( foundNodalDataQuery.size() != 1 )
      throw std::runtime_error("look_ahead_and_create::error: Too many nodal_data_base");
    BlockDataBasePostProcessing_ =  new BlockDataBasePostProcessing(*this, *foundNodalDataQuery[0]);
  }
...
Realm::load(const YAML::Node & node)
{
  ...
  if ( type_ == "multi_physics" ) {
    ...
    NaluEnv::self().naluOutputP0() << "===========================" << std::endl;
    NaluEnv::self().naluOutputP0() << "ADDED BY MJ: beam_velocity = " << solutionOptions_->beamVelocity_ << std::endl;
    NaluEnv::self().naluOutputP0() << "Stefan_Boltzmann = " << solutionOptions_->stefanBoltzmann_ << std::endl;
...
Realm::setup_post_processing_algorithms()
{
  ...
  // check for block data base query
  if ( NULL != BlockDataBasePostProcessing_ )
    BlockDataBasePostProcessing_->setup();
  ...
  for (size_t itarget=0; itarget < targetNames.size(); ++itarget) {
    ...
    for ( ii=propertyMap_.begin(); ii!=propertyMap_.end(); ++ii ) {
      ...
      switch( matData->type_) {
        ...
        //ADDED BY SEL FOR LEVEL SET METHOD
        case HEAVISIDE_MAT:
        {
          // extract the mixture fraction field
          ScalarFieldType *heaviside = metaData_->get_field<ScalarFieldType>(stk::topology::NODE_RANK, "heaviside");

          // primary and secondary
          const double propPrim = matData->primary_;
          const double propSec = matData->secondary_;
          LinearPropAlgorithm *auxAlg
            = new LinearPropAlgorithm( *this, targetPart, thePropField, heaviside, propPrim, propSec);
          propertyAlg_.push_back(auxAlg);
        }
        break;
        ...
        {
          // extract the mixture fraction field
          ScalarFieldType *temperature = metaData_->get_field<ScalarFieldType>(stk::topology::NODE_RANK, "temperature");

          // primary and secondary
          const double val0 = matData->val0_;
          const double slope = matData->slope_;
          const double ref = matData->refVal_;
    
          TempDependentPropAlgorithm *auxAlg
            = new TempDependentPropAlgorithm( *this, targetPart, thePropField, temperature, slope, val0, ref);
          propertyAlg_.push_back(auxAlg);
        }
        break;

        //ADDED BY SEL FOR LEVEL SET METHOD
        case THREE_PHASE_MAT:
        {
          // extract the mixture fraction field
          ScalarFieldType *heaviside = metaData_->get_field<ScalarFieldType>(stk::topology::NODE_RANK, "heaviside");

          // extract the mixture fraction field
          ScalarFieldType *liquidFrac = metaData_->get_field<ScalarFieldType>(stk::topology::NODE_RANK, "liquid_fraction");

          // primary and secondary
          const double propPrim = matData->primary_;
          const double propSec = matData->secondary_;
          const double propTert = matData->tertiary_;

          ThreePhasePropAlgorithm *auxAlg
            = new ThreePhasePropAlgorithm( *this, targetPart, thePropField, heaviside, liquidFrac, propPrim, propSec, propTert);
          propertyAlg_.push_back(auxAlg);
        }
        break;

        //ADDED BY ROGER FOR MULTICOMPONENT CASE 
        case MULTI_MIXTURE_MAT:
        {
            // for constant specific heat, proceed in specialty code; create the appropriate enthalpy evaluator
            if (thePropId == SPEC_HEAT_ID && needsEnthalpy_) {
                // extract reference temperature
                ...
                // check for species-based cp
                // props computed based on local mass fractions, however, constant per species k
                theCpPropEval = new SpecificHeatMultiCpkPropertyEvaluator(
                    matData->cpConstMap_, *metaData_);
                theEnthPropEval = new EnthalpyMultiCpkPropertyEvaluator(
                    *metaData_, tRef);
            ...
            //theCpPropEval = new SpecificHeatConstCpkPropertyEvaluator(matData->cpConstMap_, *metaData_);
            //theEnthPropEval = new EnthalpyMultiCpkPropertyEvaluator(matData->cpConstMap_, matData->hfConstMap_, *metaData_, 298);
            ...
            const double propPrim = matData->primary_;
            const double propSec = matData->secondary_;
            const double case_number = matData->caseNumber_;

            MultiMixturePropAlgorithm* auxAlg
                = new MultiMixturePropAlgorithm(*this, targetPart, thePropField, mixFrac, liquidFrac, temperature, propPrim, propSec, case_number);
            propertyAlg_.push_back(auxAlg);
        }
        break;

        //ADDED BY MJ FOR Questek Powderbed
        case POWDER_BED:
        {
          // extract the mixture fraction field
          ScalarFieldType *heaviside = metaData_->get_field<ScalarFieldType>(stk::topology::NODE_RANK, "heaviside");
          ...
          //extract maximum temperature to see if powder fusion happened, given max temp > solidus
          ScalarFieldType *maxTemp = metaData_->get_field<ScalarFieldType>(stk::topology::NODE_RANK, "max_temperature");
          ScalarFieldType *temperature = metaData_->get_field<ScalarFieldType>(stk::topology::NODE_RANK, "temperature");

          // primary and secondary
          const double propPowder = matData->powderProp_;
          const double propLiquid = matData->liquidProp_;
          const double propSolid = matData->solidProp_;
          const double powderSlope = matData->powder_slope_;
          const double liquidSlope = matData->liquid_slope_;
          const double solidSlope = matData->solid_slope_;
          const double liquidus = solutionOptions_->liquidus_;
          const double solidus = solutionOptions_->solidus_;

          PowderBedPropAlgorithm *auxAlg
            = new PowderBedPropAlgorithm( *this, targetPart, thePropField, heaviside,
                                           liquidFrac, temperature, maxTemp, propLiquid,
                                           propSolid, propPowder, liquidSlope, 
                                           solidSlope, powderSlope, liquidus, solidus);
          propertyAlg_.push_back(auxAlg);
        }
        break;

        //ADDED BY SEL FOR LEVEL SET METHOD
        case LINEAR_TEMP_SOLID_MAT:
        {
          // extract the mixture fraction field
          ScalarFieldType *temperature = metaData_->get_field<ScalarFieldType>(stk::topology::NODE_RANK, "temperature");

          ScalarFieldType *fL = metaData_->get_field<ScalarFieldType>(stk::topology::NODE_RANK, "liquid_fraction");
          ...
          TempDependentSolidPropAlgorithm *auxAlg
            = new TempDependentSolidPropAlgorithm( *this, targetPart, thePropField, temperature, fL, slope, val0, ref);
          propertyAlg_.push_back(auxAlg);
        }
        break;
        ...
        case LATENT_MAT:
        {
          ...
          // primary and secondary
          const double liquidus = matData->liquidus_;
          const double solidus = matData->solidus_;
          const double latent = matData->latent_;
          const double refVal = matData->refVal_;
          LatentHeatPropAlgorithm *auxAlg
            = new LatentHeatPropAlgorithm( *this, targetPart, thePropField, temperature, solidus, 
                                          liquidus, latent, refVal);
          propertyAlg_.push_back(auxAlg);
        }
        break;
...
//--------------------------------------------------------------------------
bool
Realm::get_latent_heat()
{
  return solutionOptions_->includeLatent_ ? true : false;
}
...
Realm::post_converged_work()
{
  ...
  if ( NULL != BlockDataBasePostProcessing_ )
    BlockDataBasePostProcessing_->execute();
...
//--------------------------------------------------------------------------
double
Realm::get_prandtl_number()
{
  //ADDED BY MJ for computing turbulent conductivity
  return solutionOptions_->prandtl_;
}
