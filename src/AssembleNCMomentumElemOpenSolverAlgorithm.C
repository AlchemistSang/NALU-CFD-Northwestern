...
// Class Definition
//==========================================================================
// AssembleNCMomentumElemOpenSolverAlgorithm - lhs for momentum open bc
//==========================================================================
//--------------------------------------------------------------------------
//-------- constructor -----------------------------------------------------
//--------------------------------------------------------------------------
AssembleNCMomentumElemOpenSolverAlgorithm::AssembleNCMomentumElemOpenSolverAlgorithm(
  Realm &realm,
  stk::mesh::Part *part,
...
//--------------------------------------------------------------------------
void
AssembleNCMomentumElemOpenSolverAlgorithm::initialize_connectivity()
{
...
//--------------------------------------------------------------------------
void
AssembleNCMomentumElemOpenSolverAlgorithm::execute()
{
      ...
      for ( int ip = 0; ip < numScsBip; ++ip ) {
        ...
        if ( tmdot > 0.0 ) {
          ...
          for ( int i = 0; i < nDim; ++i ) {
            ...
            p_rhs[indexR] -= rhoNN * aflux;

            // upwind lhs
            p_lhs[rowR+indexR] += rhoNN * tmdot*pecfac*alphaUpw;

            // central part
            const double fac = rhoNN*tmdot*(pecfac*om_alphaUpw+om_pecfac);
            for ( int ic = 0; ic < nodesPerFace; ++ic ) {
              const double r = p_face_shape_function[offSetSF_face+ic];
              const int nn = face_node_ordinal_vec[ic];
          ...
          for ( int i = 0; i < nDim; ++i ) {
            ...
            const double aflux = rhoNN*tmdot*(pecfac*ubipExtrapnx+om_pecfac*
                                        (nfEntrain*ubipnx + om_nfEntrain*uscsnx))*nxi
              + rhoNN*tmdot*(p_uspecBip[i] - uspecbipnx*nxi);
            ...
            for ( int j = 0; j < nDim; ++j ) {
              ...
              // upwind
              p_lhs[rowR+nearestNode*nDim+j] += rhoNN*tmdot*pecfac*alphaUpw*nxinxj;

              // central part; exposed face
              double fac = rhoNN*tmdot*(pecfac*om_alphaUpw+om_pecfac*nfEntrain)*nxinxj;
              for ( int ic = 0; ic < nodesPerFace; ++ic ) {
                const double r = p_face_shape_function[offSetSF_face+ic];
                const int nn = face_node_ordinal_vec[ic];
              ...
              fac = rhoNN*tmdot*om_pecfac*om_nfEntrain*nxinxj;
              for ( int ic = 0; ic < nodesPerElement; ++ic ) {
                const double r = p_shape_function[offSetSF_elem+ic];
