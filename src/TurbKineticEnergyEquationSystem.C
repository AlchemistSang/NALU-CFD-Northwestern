    ...
    isInit_(true)
{
  ...
  if ( (turbulenceModel_ != SST) && (turbulenceModel_ != KSGS) && (turbulenceModel_ != SST_DES) && (turbulenceModel_ != K_EPSILON)) {
    throw std::runtime_error("User has requested TurbKinEnergyEqs, however, turbulence model is not KSGS, SST, K_EPSILON or SST_DES");
  }
  ...
  stk::mesh::Part *part)
{
  ...
  if ( itsm == solverAlgDriver_->solverAlgMap_.end() ) {
    ...
    switch(turbulenceModel_) {
      ...
      break;
      default:
        throw std::runtime_error("Unsupported turbulence model in TurbKe: only SST, SST_DES, K_EPSILON and Ksgs supported");
    }
    ...
    if ( isrc != realm_.solutionOptions_->srcTermsMap_.end() ) {
      ...
      for (size_t k = 0; k < mapNameVec.size(); ++k ) {
        ...
        //MJ: Darcy source term enforces k to be zero in solid
          ...
          suppAlg = new DarcyDragTermTurbKineticEnergySrcNodeSuppAlg(realm_);
        }
        else {
  ...
  if ( itev == diffFluxCoeffAlgDriver_->algMap_.end() ) {
    ...
    switch(turbulenceModel_) {
      ...
      case K_EPSILON:
      {
        //MJ: effective viscosity for kEpsilon model = turb_visc / sigmaK
        const double sigmaK = realm_.get_turb_model_constant(TM_sigmaK_kEps);
        effDiffAlg = new EffectiveKEpsilonDiffFluxCoeffAlgorithm(realm_, part, visc_, tvisc_, evisc_, sigmaK);
      }
      break;
  ...
  const WallBoundaryConditionData &wallBCData)
{
  ...
  else if ( freeStreamApproach )
  {
    Algorithm *freeStreamDirichletAlg = NULL;
    freeStreamDirichletAlg = new ComputeFreeStreamTurbKineticAlgorithm(realm_, part, realm_.realmUsesEdges_);
    freeStreamAlg_.push_back(freeStreamDirichletAlg);
  }
//MJ: I want to apply a no flux BC on Ke for the top BC of melt pool and this statement doean't let me
/*
  else {
    throw std::runtime_error("TKE active with wall bc, however, no value of tke or wall function specified");
  }
*/
//MJ: do nothing if neither one of the blow is specified -> no flux BC
if (wallFunctionApproach || tkeSpecified || freeStreamApproach )
{
  // Dirichlet bc
  std::map<AlgorithmType, SolverAlgorithm *>::iterator itd =
      solverAlgDriver_->solverDirichAlgMap_.find(algType);
...
}// end if waalFunction of constant tke value
...
//--------------------------------------------------------------------------
//--------------- compute_freeStream_tke() ---------------------------------
//--------------------------------------------------------------------------
void
TurbKineticEnergyEquationSystem::compute_freeStream_tke()
{
