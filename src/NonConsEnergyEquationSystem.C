...
#include <NonConsEnergyEquationSystem.h>
...
// Class Definition
//==========================================================================
// NonConsEnergyEquationSystem - manages h pde system; with T as dependent var
//==========================================================================
//--------------------------------------------------------------------------
//-------- constructor -----------------------------------------------------
//--------------------------------------------------------------------------
NonConsEnergyEquationSystem::NonConsEnergyEquationSystem(
...
//--------------------------------------------------------------------------
void
NonConsEnergyEquationSystem::initial_work()
{
...
//--------------------------------------------------------------------------
void
NonConsEnergyEquationSystem::register_nodal_fields(
  stk::mesh::Part *part)
{
...
//--------------------------------------------------------------------------
void
NonConsEnergyEquationSystem::register_interior_algorithm(
  stk::mesh::Part *part)
{
...
//--------------------------------------------------------------------------
void
NonConsEnergyEquationSystem::register_inflow_bc(
  stk::mesh::Part *part,
...
//--------------------------------------------------------------------------
void
NonConsEnergyEquationSystem::register_open_bc(
  stk::mesh::Part *part,
  const stk::topology &/*theTopo*/,
...
//--------------------------------------------------------------------------
void
NonConsEnergyEquationSystem::register_wall_bc(
  stk::mesh::Part *part,
  const stk::topology &/*theTopo*/,
...
//--------------------------------------------------------------------------
void
NonConsEnergyEquationSystem::register_contact_bc(
  stk::mesh::Part *part,
  const stk::topology &theTopo,
...
//--------------------------------------------------------------------------
void
NonConsEnergyEquationSystem::register_symmetry_bc(
  stk::mesh::Part *part,
  const stk::topology &/*theTopo*/,
...
//--------------------------------------------------------------------------
void
NonConsEnergyEquationSystem::register_non_conformal_bc(
  stk::mesh::Part *part,
  const stk::topology &/*theTopo*/)
{
...
//--------------------------------------------------------------------------
void
NonConsEnergyEquationSystem::register_overset_bc()
{
...
//--------------------------------------------------------------------------
void
NonConsEnergyEquationSystem::initialize()
{
...
//--------------------------------------------------------------------------
void
NonConsEnergyEquationSystem::reinitialize_linear_system()
{
  // delete old solver
...
//--------------------------------------------------------------------------
void
NonConsEnergyEquationSystem::register_initial_condition_fcn(
  stk::mesh::Part *part,
  ...
  const std::map<std::string, std::vector<double> > &/*theParams*/)
{
  ...
  if (iterName != theNames.end()) {
    ...
    if ( fcnName == "VariableDensityNonIso" ) {
      ...
      throw std::runtime_error("NonConsEnergyEquationSystem::register_initial_condition_fcn: limited user functions supported");
    }
...
//--------------------------------------------------------------------------
void
NonConsEnergyEquationSystem::solve_and_update()
{
  // compute bc enthalpy
...
//--------------------------------------------------------------------------
void
NonConsEnergyEquationSystem::post_iter_work()
{
...
//--------------------------------------------------------------------------
void
NonConsEnergyEquationSystem::post_adapt_work()
{
  if ( realm_.process_adaptivity() ) {
    NaluEnv::self().naluOutputP0() << "--NonConsEnergyEquationSystem::post_adapt_work()" << std::endl;
    extract_temperature();
  }
}
...
//--------------------------------------------------------------------------
void
NonConsEnergyEquationSystem::extract_temperature()
{
...
//--------------------------------------------------------------------------
void
NonConsEnergyEquationSystem::post_converged_work()
{
  if ( lowSpeedCompressActive_ ) {
    stk::mesh::MetaData & meta_data = realm_.meta_data();
...
//--------------------------------------------------------------------------
void
NonConsEnergyEquationSystem::predict_state()
{
...
//--------------------------------------------------------------------------
void
NonConsEnergyEquationSystem::temperature_bc_setup(
  UserData userData,
  stk::mesh::Part *part,
  ...
  const bool copyBCVal )
{
  ...
  if ( CONSTANT_UD == theDataType ) {
    ...
    if ( fcnName == "flow_past_cylinder" ) {
      ...
      throw std::runtime_error("NonConsEnergyEquationSystem::temperature_bc_setup; limited user functions supported");
    }
  } 
  else {
    throw std::runtime_error("NonConsEnergyEquationSystem::temperature_bc_setup: only function and constants supported (and none specified)");   
  }
...
//--------------------------------------------------------------------------
void
NonConsEnergyEquationSystem::manage_projected_nodal_gradient(
  EquationSystems& eqSystems)
{
...
//--------------------------------------------------------------------------
void
NonConsEnergyEquationSystem::compute_projected_nodal_gradient()
{
  if ( !managePNG_ ) {
