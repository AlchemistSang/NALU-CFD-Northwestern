...
// Class Definition
//==========================================================================
// AssembleNCMixtureScalarElemSolverAlgorithm - add LHS/RHS for scalar
//==========================================================================
//--------------------------------------------------------------------------
//-------- constructor -----------------------------------------------------
//--------------------------------------------------------------------------
AssembleNCMixtureScalarElemSolverAlgorithm::AssembleNCMixtureScalarElemSolverAlgorithm(
  Realm &realm,
  stk::mesh::Part *part,
  ...
  : SolverAlgorithm(realm, part, eqSystem),
    ...
    scalarQ_(scalarQ),
    scalarQLadv_(scalarQLadv),
    scalarQL_(scalarQL),
    scalarQS_(scalarQS),
    scalarQLold_(scalarQLold),
    scalarQSold_(scalarQSold),
    scalarFL_(scalarFL),
    dqdx_(dqdx),
...
//--------------------------------------------------------------------------
void
AssembleNCMixtureScalarElemSolverAlgorithm::initialize_connectivity()
{
...
//--------------------------------------------------------------------------
void
AssembleNCMixtureScalarElemSolverAlgorithm::execute()
{
  ...
  std::vector<double> ws_scalarQLadv;
  std::vector<double> ws_scalarQL;
  std::vector<double> ws_scalarQS;
  std::vector<double> ws_scalarQLold;
  std::vector<double> ws_scalarQSold;
  ...
  for ( stk::mesh::BucketVector::const_iterator ib = elem_buckets.begin();
    ...
    ws_scalarQLadv.resize(nodesPerElement);
    ws_scalarQL.resize(nodesPerElement);
    ws_scalarQS.resize(nodesPerElement);
    ws_scalarQLold.resize(nodesPerElement);
    ws_scalarQSold.resize(nodesPerElement);
    ...
    double* p_scalarQLadv = &ws_scalarQLadv[0];
    double* p_scalarQL = &ws_scalarQL[0];
    double* p_scalarQS = &ws_scalarQS[0];
    double* p_scalarQLold = &ws_scalarQLold[0];
    double* p_scalarQSold = &ws_scalarQSold[0];
    ...
    for ( stk::mesh::Bucket::size_type k = 0 ; k < length ; ++k ) {
      ...
      for ( int ni = 0; ni < num_nodes; ++ni ) {
        ...
        p_scalarQLadv[ni]   = *stk::mesh::field_data(*scalarQLadv_, node);
        p_scalarQL[ni]      = *stk::mesh::field_data(*scalarQL_, node);
        p_scalarQS[ni]      = *stk::mesh::field_data(*scalarQS_, node);
        p_scalarQLold[ni]   = *stk::mesh::field_data(*scalarQLold_, node);
        p_scalarQSold[ni]   = *stk::mesh::field_data(*scalarQSold_, node);
...
/*        p_ubar[0] = 2.0 * M_PI * p_coordIp[1];
        ...
        if ( useLimiter ) {
          ...
          const double dq = p_scalarQLadv[ir] - p_scalarQLadv[il];
        ...
        const double qIpL = p_scalarQLadv[il] + dqL * hoUpwind * limitL;
        const double qIpR = p_scalarQLadv[ir] - dqR * hoUpwind * limitR;
        ...
        if (p_scalarFL[il] > 0.0 && p_scalarFL[il] < 1.0) {
            // Go back first
            p_lhs[rowL + il] -= alhsfacL * rhoL;
            p_lhs[rowR + il] += alhsfacL * rhoR;

            //double para_M_il = p_scalarQLold[il] / p_scalarQSold[il];
            double para_M_il = M_value;
            p_lhs[rowL + il] += alhsfacL / (p_scalarFL[il] + (1.0 - p_scalarFL[il]) / para_M_il) * rhoL;
            p_lhs[rowR + il] -= alhsfacL / (p_scalarFL[il] + (1.0 - p_scalarFL[il]) / para_M_il) * rhoR;
        }
        ...
        if (p_scalarFL[ir] > 0.0 && p_scalarFL[ir] < 1.0) {
            // Go back first  
            p_lhs[rowL + ir] -= alhsfacR * rhoL;
            p_lhs[rowR + ir] += alhsfacR * rhoR;

            //double para_M_ir = p_scalarQLold[ir] / p_scalarQSold[ir];
            double para_M_ir = M_value;
            p_lhs[rowL + ir] += alhsfacR / (p_scalarFL[ir] + (1.0 - p_scalarFL[ir]) / para_M_ir) * rhoL;
            p_lhs[rowR + ir] -= alhsfacR / (p_scalarFL[ir] + (1.0 - p_scalarFL[ir]) / para_M_ir) * rhoR;
        }
        ...
        for ( int ic = 0; ic < nodesPerElement; ++ic ) {
          ...
          if (p_scalarFL[ic] > 0.0 && p_scalarFL[ic] < 1.0) {
              // Go back first
              p_lhs[rowL + ic] -= lhsfacAdv * rhoL;
              p_lhs[rowR + ic] += lhsfacAdv * rhoR;

              //double para_M_ic = p_scalarQLold[ic] / p_scalarQSold[ic];
              double para_M_ic = M_value;
              p_lhs[rowL + ic] += lhsfacAdv / (p_scalarFL[ic] + (1.0 - p_scalarFL[ic]) / para_M_ic) * rhoL;
              p_lhs[rowR + ic] -= lhsfacAdv / (p_scalarFL[ic] + (1.0 - p_scalarFL[ic]) / para_M_ic) * rhoR;
          }
...
//--------------------------------------------------------------------------
double
AssembleNCMixtureScalarElemSolverAlgorithm::van_leer(
  const double &dqm,
  const double &dqp,
  const double &small)
{
