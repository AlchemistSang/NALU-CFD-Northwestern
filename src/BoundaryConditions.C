...
BoundaryCondition * BoundaryCondition::load(const YAML::Node & node) 
{
  if ( node.FindValue("wall_boundary_condition") ){
    ...
    NaluEnv::self().naluOutputP0() << "Wall BC name:        " << wallBC.bcName_
                    << " on " << wallBC.targetName_;

   // MJ: making sure Marangoni and evaporation have been registered
    WallUserData userData = wallBC.userData_;
    if (userData.evapHeatSpec_)
          NaluEnv::self().naluOutputP0() << "  Evaporation_cooling";

    if (userData.evapMomentumSpec_)
          NaluEnv::self().naluOutputP0() << "  Recoil_Pressure";

    if (userData.evapContinuitySpec_)
          NaluEnv::self().naluOutputP0() << "  Evaporation_Mass";

    if (userData.marangoniSpec_)
          NaluEnv::self().naluOutputP0() << "  Marangoni";
