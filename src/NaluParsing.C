...
//MJ: epsilon in k-epsilon model
void operator >> (const YAML::Node& node, TurbKinDissRate& TKEdr) {
  node >> TKEdr.TurbKinDissRate_;
}
...
void operator >> (const YAML::Node& node, LevelSet& phi) {
  node >> phi.levelSet_;
}

void operator >> (const YAML::Node& node, VolumeFraction& H) {
  node >> H.volume_fraction_;
}

void operator >> (const YAML::Node& node, Intensity& I) {
  node >> I.intensity_;
}
...
void operator >> (const YAML::Node& node, WallUserData& wallData) {
  ...
  //MJ: we assume that epsilon in k-epsilon can have a Dirichlet BC on a wall
  // and does not necessarily have to be compued based on wall function 
  if ( node.FindValue("tke_dissipation_rate") ) {
    node["tke_dissipation_rate"] >> wallData.TKEdr_;
    wallData.bcDataSpecifiedMap_["tke_dissipation_rate"] = true;
    wallData.bcDataTypeMap_["tke_dissipation_rate"] = CONSTANT_UD;
  }
  ...
  if ( node.FindValue("level_set") ) {
    node["level_set"] >> wallData.levelSet_;
    wallData.bcDataSpecifiedMap_["level_set"] = true;
    wallData.bcDataTypeMap_["level_set"] = CONSTANT_UD;
  }
  if ( node.FindValue("volume_fraction") ) {
    node["volume_fraction"] >> wallData.volume_fraction_;
    wallData.bcDataSpecifiedMap_["volume_fraction"] = true;
    wallData.bcDataTypeMap_["volume_fraction"] = CONSTANT_UD;
  }
  if ( node.FindValue("intensity") ) {
    node["intensity"] >> wallData.intensity_;
    wallData.bcDataSpecifiedMap_["intensity"] = true;
    wallData.bcDataTypeMap_["intensity"] = CONSTANT_UD;
  }
  ...
  // not appropriate
  // MJ: omeag from SST RANS model does not get a Dirichlet BC on a wall
  ...
  // MJ: laser boundary condition, which is a variable heat flux imposed on the top wall BC
  if ( node.FindValue("laser_heat_flux") ) {
    node["laser_heat_flux"] >> wallData.laserHeatFluxSpec_;
    wallData.laserHeatFluxSpec_ = true;
  }

  // MJ: applying a Dirichlet BC for ka nd epsilon (turbulence model) on the top of melt pool
  if ( node.FindValue("free_stream_turbulence") ) {
    node["free_stream_turbulence"] >> wallData.freeStreamDirichlet_;
    wallData.freeStreamDirichlet_ = true;
  }

  // MJ: marangoni BC applied on the boundary that the laser flux is imposed on
  // when marangoni is on, there should be no velocity specified on the wall
  if ( node.FindValue("marangoni") ) {
    node["marangoni"] >> wallData.marangoniSpec_;
    wallData.marangoniSpec_ = true;
  }

  // MJ: applying evaporation to the heat eqaution as a BC
  if ( node.FindValue("evaporation_cooling") ) {
    node["evaporation_cooling"] >> wallData.evapHeatSpec_;
    wallData.evapHeatSpec_ = true;
  }

  // MJ: accounting for evaporation mass loss in continuity
  if ( node.FindValue("evaporation_mass") ) {
    node["evaporation_mass"] >> wallData.evapContinuitySpec_;
    wallData.evapContinuitySpec_ = true;
  }

  // MJ: applying recoil pressure to the momentum eqaution as a BC
  if ( node.FindValue("recoil_pressure") ) {
    node["recoil_pressure"] >> wallData.evapMomentumSpec_;
    wallData.evapMomentumSpec_ = true;
  }
}
...
void operator >> (const YAML::Node& node, InflowUserData& inflowData) {
  ...
  if ( node.FindValue("tke_dissipation_rate") ){
    node["tke_dissipation_rate"] >> inflowData.TKEdr_;
    inflowData.TKEdrSpec_ = true;
  }
  if ( node.FindValue("mixture_fraction") ){
  ...
  if ( node.FindValue("level_set") ) {
    node["level_set"] >> inflowData.levelSet_;
    inflowData.levelSetSpec_ = true;
  }
  if ( node.FindValue("volume_fraction") ) {
   node["volume_fraction"] >> inflowData.volume_fraction_;
   inflowData.volume_fractionSpec_ = true;
  }
  if ( node.FindValue("intensity") ) {
   node["intensity"] >> inflowData.intensity_;
   inflowData.intensitySpec_ = true;
  }
...
void operator >> (const YAML::Node& node, OpenUserData& openData) {
  ...
  if ( node.FindValue("tke_dissipation_rate") ){
    node["tke_dissipation_rate"] >> openData.TKEdr_;
    openData.TKEdrSpec_ = true;
  }
  if ( node.FindValue("pressure") ) {
  ...
  if ( node.FindValue("level_set") ) {
    node["level_set"] >> openData.levelSet_;
    openData.levelSetSpec_ = true;
  }
  if ( node.FindValue("volume_fraction") ) {
    node["volume_fraction"] >> openData.volume_fraction_;
    openData.volume_fractionSpec_ = true;
  }
  if ( node.FindValue("intensity") ) {
    node["intensity"] >> openData.intensity_;
    openData.intensitySpec_ = true;
  }
}
