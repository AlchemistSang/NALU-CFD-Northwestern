...
// nalu
#include <AssembleNCContinuityBoundSolverAlgorithm.h>
...
// Class Definition
//==========================================================================
// AssembleNCContinuityBoundSolverAlgorithm - lhs for continuity open bc
//==========================================================================
//--------------------------------------------------------------------------
//-------- constructor -----------------------------------------------------
//--------------------------------------------------------------------------
AssembleNCContinuityBoundSolverAlgorithm::AssembleNCContinuityBoundSolverAlgorithm(
  Realm &realm,
  stk::mesh::Part *part,
...
//--------------------------------------------------------------------------
void
AssembleNCContinuityBoundSolverAlgorithm::initialize_connectivity()
{
...
//--------------------------------------------------------------------------
void
AssembleNCContinuityBoundSolverAlgorithm::execute()
{
  ...
  for ( stk::mesh::BucketVector::const_iterator ib = face_buckets.begin();
    ...
    ws_deriv.resize(nDim*numScsBip*nodesPerElement);
    ...
    double *p_Pressure = &ws_bcPressure[0];
      ...
      for ( int ip = 0; ip < numScsBip; ++ip ) {
        ...
        for ( int ic = 0; ic < nodesPerFace; ++ic ) {
          ...
          for ( int j = 0; j < nDim; ++j ) {
            ...
            p_dpdxIp[j] += p_dndx[offSetDnDx+j] * p_Pressure[ic];
            lhsfac += -p_dndx[offSetDnDx+j] * axj;
          }
        ...
        for ( int j = 0; j < nDim; ++j ) {
          const double axj = areaVec[ip*nDim+j];
          mdot += -projTimeScale*(p_dpdxIp[j] - p_GpdxBip[j])*axj;
        }
