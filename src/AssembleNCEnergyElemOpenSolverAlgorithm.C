...
// Class Definition
//==========================================================================
// AssembleNCEnergyElemOpenSolverAlgorithm - lhs for scalar open bc
//==========================================================================
//--------------------------------------------------------------------------
//-------- constructor -----------------------------------------------------
//--------------------------------------------------------------------------
AssembleNCEnergyElemOpenSolverAlgorithm::AssembleNCEnergyElemOpenSolverAlgorithm(
  Realm &realm,
  stk::mesh::Part *part,
...
//--------------------------------------------------------------------------
void
AssembleNCEnergyElemOpenSolverAlgorithm::initialize_connectivity()
{
...
//--------------------------------------------------------------------------
void
AssembleNCEnergyElemOpenSolverAlgorithm::execute()
{
  ...
  for ( stk::mesh::BucketVector::const_iterator ib = face_buckets.begin();
    ...
    for ( stk::mesh::Bucket::size_type k = 0 ; k < length ; ++k ) {
      ...
      for ( int ip = 0; ip < numScsBip; ++ip ) {
        ...
        const double densNN       = *stk::mesh::field_data(densityNp1, nodeR);
        const double cpNN       = *stk::mesh::field_data(cpNp1, nodeR);
        const double diffCoeffL  = *stk::mesh::field_data(*thermalCond_, nodeL);
        const double diffCoeffR  = *stk::mesh::field_data(*thermalCond_, nodeR);
        ...
        if ( tmdot > 0.0 ) {
          ...
          // total advection
          const double aflux = cpNN * densNN * tmdot*(pecfac*qUpwind+om_pecfac*qIp);

          p_rhs[nearestNode] -= aflux;

          // upwind lhs
          p_lhs[rowR+nearestNode] += cpNN * densNN * tmdot*pecfac*alphaUpw;

          // central part
          const double fac = cpNN * densNN * tmdot*(pecfac*om_alphaUpw+om_pecfac);
          for ( int ic = 0; ic < nodesPerFace; ++ic ) {
            const double r = p_face_shape_function[offSetSF_face+ic];
          ...
          const double aflux = cpNN * densNN * tmdot*qIpEntrain;
          p_rhs[nearestNode] -= aflux;
        }
      }
