...
SolutionOptions::SolutionOptions()
  : hybridDefault_(0.0),
    ...
    includeLatent_(false),
    balancedForce_(false),
    addRecoil_(false),
    projectDarcy_(false),
    redistCorrection_(true),
    liquidus_(0.0),
    solidus_(0.0),
    latent_(0.0),
    darcyBig_(0.0),
    ...
    beamVelocity_(0.0),
    vofEps_(1.0),
    redistPenalty_(1.0e4),
    toolFileName_("null"),
    powderFileName_("null"),
    gasConstant_(1.0),
    latentEvap_(0.0),
    ambientP_(0.0),
    sigma0_(0.0),
    ...
    molarMass_(0.0),
    prandtl_(0.0),
    numNonLinearVolCorrectionIter_(4),
    substrateLevel_(0.0),
    keyholeDepth_(0.0),
    keyholeRadius_(0.0)
{
...
SolutionOptions::load(const YAML::Node & y_node)
{
  ...
  if(y_solution_options)
  {
    ...
    // latent heat procedure 
    get_if_present(*y_solution_options, "include_latent_heat", includeLatent_, includeLatent_);

    // balanced force algorithm
    get_if_present(*y_solution_options, "include_balanced_force", balancedForce_, balancedForce_);

    // balanced force algorithm with 3 phases
    get_if_present(*y_solution_options, "project_darcy_term", projectDarcy_, projectDarcy_);

    // local volume correction during redistancing
    get_if_present(*y_solution_options, "local_volume_correction", redistCorrection_, redistCorrection_);
    ...
    if (y_options)
    {
      for (size_t ioption = 0; ioption < y_options->size(); ++ioption)
      {
        ...
        if (expect_map(y_option, "hybrid_factor", optional)) {
          ...
          // Add in solution options for energy conservation
          get_if_present(y_user_constants, "liquidus_temperature",  liquidus_, liquidus_);
          get_if_present(y_user_constants, "solidus_temperature",  solidus_, solidus_);
          get_if_present(y_user_constants, "latent_heat",  latent_, latent_);
          get_if_present(y_user_constants, "sigma", sigma0_, sigma0_);
          get_if_present(y_user_constants, "dsigdt", dsigdt_, dsigdt_);
          get_if_present(y_user_constants, "darcy_big", darcyBig_, darcyBig_);
          get_if_present(y_user_constants, "darcy_small", darcySmall_, darcySmall_);
          get_if_present(y_user_constants, "beam_power", beamPower_, beamPower_);
          get_if_present(y_user_constants, "beam_radius", beamRadius_, beamRadius_);
          get_if_present(y_user_constants, "beam_efficiency", beamEff_, beamEff_);
          // MJ: specifies the thickness of the powder layer for Questek simulations
          get_if_present(y_user_constants, "powderLayer_depth", powderDepth_, powderDepth_);
          // MJ: adding laser velocity, will be used to advect variables 
          get_if_present(y_user_constants, "beam_velocity", beamVelocity_, beamVelocity_);
          // MJ: Prandtl number will be used to compute turb conductivity based on turb viscosity 
          get_if_present(y_user_constants, "prandtl", prandtl_, prandtl_);
          get_if_present(y_user_constants, "volume_correct_iter", numNonLinearVolCorrectionIter_, 
                                                                  numNonLinearVolCorrectionIter_);
          // MJ: approximate melt pool length will be used for kepsilon top BC
          get_if_present(y_user_constants, "meltpool_length", meltLength_, meltLength_);

          get_if_present(y_user_constants, "toolpath_file", toolFileName_, toolFileName_);
          get_if_present(y_user_constants, "powder_file", powderFileName_, powderFileName_);
          get_if_present(y_user_constants, "vof_eps", vofEps_, vofEps_);
          get_if_present(y_user_constants, "redistancing_penalty", redistPenalty_, redistPenalty_);
          // evaporation constants
          get_if_present(y_user_constants, "boltzmann_constant", kB_, kB_);
          get_if_present(y_user_constants, "emissivity", emissivity_, emissivity_);
          get_if_present(y_user_constants, "molecular_weight", molecularWeight_, molecularWeight_);
          get_if_present(y_user_constants, "ambient_pressure", ambientP_, ambientP_);
          get_if_present(y_user_constants, "ambient_temperature", ambientTemp_, ambientTemp_);
          get_if_present(y_user_constants, "evaporation_temp", evapTemp_, evapTemp_);
          get_if_present(y_user_constants, "latent_evaporation", latentEvap_, latentEvap_);
          get_if_present(y_user_constants, "gas_constant", gasConstant_, gasConstant_);
          get_if_present(y_user_constants, "molar_mass", molarMass_, molarMass_);
          get_if_present(y_user_constants, "substrate_level", substrateLevel_, substrateLevel_);
...
SolutionOptions::initialize_turbulence_constants() 
{
  ...
  turbModelConstantMap_[TM_tkeProdLimitRatio] = (turbulenceModel_ == SST || turbulenceModel_ == SST_DES || turbulenceModel_ == K_EPSILON) ? 10.0 : 500.0;

  //MJ: this factor prevents from turb viscosity getting huge in the stagnation points
  if (turbulenceModel_ == K_EPSILON )
    turbModelConstantMap_[TM_tkeProdLimitRatio] = 10.0;
  ...
  turbModelConstantMap_[TM_cMu_kEps] = 0.09;
  turbModelConstantMap_[TM_sigmaK_kEps] = 1.0;
  turbModelConstantMap_[TM_sigmaEps_kEps] = 1.3;
  turbModelConstantMap_[TM_cOneEps_kEps] = 1.44;
  turbModelConstantMap_[TM_cTwoEps_kEps] = 1.92;
}
