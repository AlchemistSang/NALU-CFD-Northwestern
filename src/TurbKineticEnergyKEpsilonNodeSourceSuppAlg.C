...
// Class Definition
//==========================================================================
// TurbKineticEnergyKEpsilonNodeSourceSuppAlg - source terms for K eq from K epsilon
//==========================================================================
//--------------------------------------------------------------------------
//-------- constructor -----------------------------------------------------
//--------------------------------------------------------------------------
  ...
  stk::mesh::Entity node)
{
  ...
  for ( int i = 0; i < nDim; ++i ) {
    ...
    for ( int j = 0; j < nDim; ++j ) {
      Sij2 += dudx[offSet+j]*(dudx[offSet+j] + dudx[nDim*j+i]);
    }
  }
  double Pk = Sij2 * tvisc;

  //double d_pk_dtke = 2.0*rho*cMu_*Sij2*tke / std::max(epsilon, 1.0e-5);

  double Dk = rho*epsilon;

  if ( Pk > tkeProdLimitRatio_*Dk )
    Pk = tkeProdLimitRatio_*Dk;

  rhs[0] += (Pk - Dk)*dualVolume;
  //lhs[0] -= d_pk_dtke*dualVolume;
}
