...
// Class Definition
//==========================================================================
// AssembleHeaviElemSolverAlgorithm - add LHS/RHS for scalar
//==========================================================================
//--------------------------------------------------------------------------
//-------- constructor -----------------------------------------------------
//--------------------------------------------------------------------------
AssembleHeaviElemSolverAlgorithm::AssembleHeaviElemSolverAlgorithm(
  Realm &realm,
  stk::mesh::Part *part,
  ...
  : SolverAlgorithm(realm, part, eqSystem),
    ...
    compressionFactor_(compressionFactor)
{
  // save off fields
  ...
  compressVel_ = meta_data.get_field<VectorFieldType>(stk::topology::NODE_RANK, "compress_vel");
...
//--------------------------------------------------------------------------
void
AssembleHeaviElemSolverAlgorithm::initialize_connectivity()
{
...
//--------------------------------------------------------------------------
void
AssembleHeaviElemSolverAlgorithm::execute()
{
  ...
  double eps = realm_.solutionOptions_->vofEps_;
  double velocity = 1.0;
  ...
  for ( stk::mesh::BucketVector::const_iterator ib = elem_buckets.begin();
    ...
    double p_uIp[nDim];
    double p_gradH[nDim];
    double p_compressIp[nDim];
    double p_compress[nDim*nodesPerElement];
    double p_levelSet[nodesPerElement];
    double p_perm[nodesPerElement];
    ...
    for ( stk::mesh::Bucket::size_type k = 0 ; k < length ; ++k ) {
      ...
      for ( int ni = 0; ni < num_nodes; ++ni ) {
        ...
        const double * compressVel     = stk::mesh::field_data(*compressVel_, node );

        // gather scalars
        p_scalarQNp1[ni]    = *stk::mesh::field_data(scalarQNp1, node );
        p_density[ni]       = *stk::mesh::field_data(densityNp1, node );
        p_levelSet[ni]       = *stk::mesh::field_data(levelSetNp1, node );
        p_perm[ni]       = *stk::mesh::field_data(*permeability_, node );
        ...
        for ( int i=0; i < nDim; ++i ) {
          ...
          p_compress[niNdim+i] = compressVel[i];
      ...
      for ( int ip = 0; ip < numScsIp; ++ip ) {
        ...
        for ( int ic = 0; ic < nodesPerElement; ++ic ) {
          ...
          kIp += r * (1.0/(1.0+p_perm[ic]));
          // compute scs point values
	  const int offSetDnDx = nDim*nodesPerElement*ip + ic*nDim;
          for ( int i = 0; i < nDim; ++i ) {
            p_coordIp[i] += r*p_coordinates[ic*nDim+i];
            p_compressIp[i] += r*p_compress[ic*nDim+i];
            p_uIp[i] += r * p_vrtm[ic*nDim+i];
            p_gradH[i] += p_dndx[offSetDnDx+i]* p_levelSet[ic];
          }
        }

        double normGradH = std::sqrt( p_gradH[0] * p_gradH[0] + 
                                      p_gradH[1] * p_gradH[1] + 
                                      p_gradH[2] * p_gradH[2] );
        if (normGradH < 1.0e-8)
        {
        ...
        {
          p_gradH[0] = p_gradH[0]/normGradH;
          p_gradH[1] = p_gradH[1]/normGradH;
          p_gradH[2] = p_gradH[2]/normGradH;
        }
        // calculate compressive flux
        double cFlux = 0.0;
        for (int j = 0; j < nDim; j++)
        {
          //cFlux += (1.0 - qIp) * p_gradH[j] * p_scs_areav[ip*nDim+j];
          cFlux += p_compressIp[j] * p_scs_areav[ip*nDim+j];
        }

        double magUIp = compressionFactor_;
        // Scale mdot by density by density
        const double tmdot = (mdot[ip] + kIp * magUIp * cFlux);
...
//        p_ubar[2] = 0.0; // END FIX HERE
/*        double xip = p_coordIp[0];
        double yip = p_coordIp[1];
        double zip = p_coordIp[2];
        p_ubar[0] = 2.0 * sin(M_PI * xip) * sin(M_PI * xip) * 
                            sin(2.0 * M_PI * yip) * 
                            sin(2.0 * M_PI * zip) *
                            cos(M_PI * currTime / 3.0);
        p_ubar[1] = - sin ( 2.0 * M_PI * xip ) *
                        sin ( M_PI * yip ) * sin( M_PI * yip ) *
                        sin ( 2.0 * M_PI * zip) *
                        cos ( M_PI * currTime / 3.0);
        p_ubar[2] = - sin ( 2.0 * M_PI * xip) *
                        sin ( 2.0 * M_PI * yip) *
                        sin ( M_PI * zip) * sin(M_PI * zip) *
                        cos ( M_PI * currTime / 3.0);
        ...
        for ( int ic = 0; ic < nodesPerElement; ++ic ) {
          ...
          const int offSetDnDx = nDim*nodesPerElement*ip + ic*nDim;
          for ( int j = 0; j < nDim; ++j ) {
            lhsfacDiff += - kIp * magUIp * eps * p_dndx[offSetDnDx+j]*p_scs_areav[ip*nDim+j];
          }
...
//--------------------------------------------------------------------------
double
AssembleHeaviElemSolverAlgorithm::van_leer(
  const double &dqm,
  const double &dqp,
  const double &small)
{
