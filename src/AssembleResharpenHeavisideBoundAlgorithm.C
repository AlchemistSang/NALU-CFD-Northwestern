...
//-------- constructor -----------------------------------------------------
//--------------------------------------------------------------------------
AssembleResharpenHeavisideBoundAlgorithm::AssembleResharpenHeavisideBoundAlgorithm(
  Realm &realm,
  stk::mesh::Part *part,
...
//--------------------------------------------------------------------------
void
AssembleResharpenHeavisideBoundAlgorithm::execute()
{
  stk::mesh::MetaData & meta_data = realm_.meta_data();
  ...
  for ( stk::mesh::BucketVector::const_iterator ib = face_buckets.begin();
    ...
    for ( stk::mesh::Bucket::size_type k = 0 ; k < length ; ++k ) {
      ...
      for (int ni = 0; ni < num_nodes; ++ni)
      {
        ...
        // gather scalars
        ws_phi0[ni]   = *stk::mesh::field_data(*phi0_, node);
      ...
      for (int ip = 0; ip < numScsIp; ++ip)
      {
        ...
        const double phiNN = *stk::mesh::field_data(*phi0_, nodeNN);
        ...
        {
          phiIp += p_shape_function[offSet+ic] * ws_phi0[ic];
          for ( int i = 0; i < nDim; i++)
          {
