...
// Class Definition
//==========================================================================
// AssembleScalarBoundDiskSolverAlgorithm - lhs for scalar open bc
//==========================================================================
//--------------------------------------------------------------------------
//-------- constructor -----------------------------------------------------
//--------------------------------------------------------------------------
AssembleScalarBoundDiskSolverAlgorithm::AssembleScalarBoundDiskSolverAlgorithm(
  Realm &realm,
  stk::mesh::Part *part,
...
//--------------------------------------------------------------------------
void
AssembleScalarBoundDiskSolverAlgorithm::initialize_connectivity()
{
...
//--------------------------------------------------------------------------
void
AssembleScalarBoundDiskSolverAlgorithm::execute()
{
  ...
  for ( stk::mesh::BucketVector::const_iterator ib = face_buckets.begin();
    ...
    double currTime = realm_.get_current_time();
    ...
    for ( stk::mesh::Bucket::size_type k = 0 ; k < length ; ++k ) {
      ...
      for ( int ip = 0; ip < numScsBip; ++ip ) {
        ...
        // FIXME: READJUST tmdot HERE
        double p_ubarIp[3];
        double tmdot = 0.0;
        p_ubarIp[0] =  2.0 * M_PI * p_coordBip[1];
        p_ubarIp[1] = -2.0 * M_PI * p_coordBip[0];
        p_ubarIp[2] = 0.0;
        double xip = p_coordBip[0]; 
        double yip = p_coordBip[1];
        double zip = p_coordBip[2];
/*        p_ubarIp[0] = 2.0 * sin(M_PI * xip) * sin(M_PI * xip) * 
                            sin(2.0 * M_PI * yip) * 
                            sin(2.0 * M_PI * zip) *
                            cos(M_PI * currTime / 3.0);
        p_ubarIp[1] = - sin ( 2.0 * M_PI * xip ) *
                        sin ( M_PI * yip ) * sin( M_PI * yip ) *
                        sin ( 2.0 * M_PI * zip) *
                        cos ( M_PI * currTime / 3.0);
        p_ubarIp[2] = - sin ( 2.0 * M_PI * xip) *
                        sin ( 2.0 * M_PI * yip) *
                        sin ( M_PI * zip) * sin(M_PI * zip) *
                        cos ( M_PI * currTime / 3.0);*/

        for (int j = 0; j < nDim; ++j){
          tmdot += p_ubarIp[j] * areaVec[ip*nDim+j];
        }//end for(i)
        // END OF FIX
        ...
        if ( tmdot > 0.0 ) {
          ...
          // Use Integration Point Values for advection flux
          const double aflux = tmdot*qIp;
          p_rhs[nearestNode] -= aflux; 

          for (int ic = 0; ic < nodesPerFace; ++ic){
            const double r = p_face_shape_function[offSetSF_face+ic];
            const int nn = face_node_ordinal_vec[ic];
            p_lhs[rowR+nn] += tmdot * r;
          }
