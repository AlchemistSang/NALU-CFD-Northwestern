...
LowMachEquationSystem::solve_and_update()
{
  ...
  // Check for level set coupling - possibly need to calculate gradients here.
  if (momentumEqSys_->isLevelSet_)
  {
    momentumEqSys_->heaviside_ = meta_data.get_field<ScalarFieldType>(stk::topology::NODE_RANK, "heaviside");
...
//          suppAlg = new SurfaceTensionLSMomentumSrcElemSuppAlg(realm_);
//          isLevelSet_ = true;
//        }
...
//--------------------------------------------------------------------------
void
MomentumEquationSystem::compute_nodal_dphidx()
{
  const double timeA = -stk::cpu_time();
