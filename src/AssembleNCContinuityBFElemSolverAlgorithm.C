...
// Class Definition
//==========================================================================
// AssembleNCContinuityBFElemSolverAlgorithm - add LHS/RHS for continuity
//==========================================================================
//--------------------------------------------------------------------------
//-------- constructor -----------------------------------------------------
//--------------------------------------------------------------------------
AssembleNCContinuityBFElemSolverAlgorithm::AssembleNCContinuityBFElemSolverAlgorithm(
  Realm &realm,
  stk::mesh::Part *part,
...
//--------------------------------------------------------------------------
void
AssembleNCContinuityBFElemSolverAlgorithm::initialize_connectivity()
{
...
//--------------------------------------------------------------------------
void
AssembleNCContinuityBFElemSolverAlgorithm::execute()
{
  ...
  for ( stk::mesh::BucketVector::const_iterator ib = elem_buckets.begin();
    ...
    for ( stk::mesh::Bucket::size_type k = 0 ; k < length ; ++k ) {
      ...
      for ( int ni = 0; ni < num_nodes; ++ni ) {
        ...
        for ( int j=0; j < nDim; ++j ) {
          ...
          p_csfNp1[niNdim + j] = csfNp1[j];
          p_csf[niNdim + j] = csf[j];
	  ...
	  const int offSetTensor = ic * nDim * nDim;
          ...
          for ( int j = 0; j < nDim; ++j ) {
            ...
            p_invrhoGpdx[j] += r* ( p_Gpdx[nDim*ic+j]/nodalRho - p_csfNp1[nDim*ic+j]/nodalRho);
            p_uIp[j] += r*p_vrtm[nDim*ic+j];
            p_rho_uIp[j] += r*nodalRho*p_vrtm[nDim*ic+j];
            p_dpdxIp[j] += p_dndx[offSetDnDx+j]* (nodalPressure);
            lhsfac += -p_dndx_lhs[offSetDnDx+j]*p_scs_areav[ip*nDim+j]*invrhoIp;
	...
	p_csfNp1Ip[2] = ( csfCoeffIp * dHdXIp[2]) * scaleFac;
        ...
        for ( int j = 0; j < nDim; ++j ) {
          mdot += (p_uIp[j] 
                   - projTimeScale*( invrhoIp * p_dpdxIp[j] - invrhoIp * p_csfNp1Ip[j] 
