  ...
  if (y_average) {    
    ...
    if (y_specs) {
      for (size_t ispec = 0; ispec < y_specs->size(); ++ispec) {
        ...
        get_if_present(y_spec, "compute_divergence", avInfo->computeDivergence_, avInfo->computeDivergence_);
...
TurbulenceAveragingPostProcessing::setup()
{
  ...
  for (size_t k = 0; k < averageInfoVec_.size(); ++k ) {
    ...
    for ( size_t itarget = 0; itarget < avInfo->targetNames_.size(); ++itarget ) {
      ...
      // MJ: compute divergence to make sure the velocity field is div-free
      if ( avInfo->computeDivergence_ ) {
        const std::string divergenceName = "divergence";
        ScalarFieldType *divergenceField = &(metaData.declare_field<ScalarFieldType>(stk::topology::NODE_RANK, divergenceName));
        stk::mesh::put_field(*divergenceField, *targetPart);
      }
  ...
  const AveragingInfo *avInfo)
{
  ...
  if ( avInfo->computeDivergence_ ) {
    NaluEnv::self().naluOutputP0() << "Divergence will be computed; add divergence to output"<< std::endl;
  }
...
TurbulenceAveragingPostProcessing::execute()
{
  ...
  for (size_t k = 0; k < averageInfoVec_.size(); ++k ) {
    ...
    // MJ: process velocity field divergence
    if ( avInfo->computeDivergence_ ) {
      compute_divergence(avInfo->name_, s_all_nodes);
    }
  }
}
...
//--------------------------------------------------------------------------
void
TurbulenceAveragingPostProcessing::compute_divergence(
  const std::string &averageBlockName,
  stk::mesh::Selector s_all_nodes)
{
  stk::mesh::MetaData & metaData = realm_.meta_data();
  ...
  const int nDim = realm_.spatialDimension_;
  const std::string DivFieldName = "divergence";
  
  // extract fields
  stk::mesh::FieldBase *dudx_ = metaData.get_field(stk::topology::NODE_RANK, "dudx");
  stk::mesh::FieldBase *Div = metaData.get_field(stk::topology::NODE_RANK, DivFieldName);

  stk::mesh::BucketVector const& node_buckets_div =
    realm_.get_buckets( stk::topology::NODE_RANK, s_all_nodes );
  for ( stk::mesh::BucketVector::const_iterator ib = node_buckets_div.begin();
        ib != node_buckets_div.end() ; ++ib ) {
    stk::mesh::Bucket & b = **ib ;
    const stk::mesh::Bucket::size_type length   = b.size();
    
    // fields
    const double * du = (double*)stk::mesh::field_data(*dudx_, b);
    double *divergence_ = (double*)stk::mesh::field_data(*Div,b);
    const int offSet = nDim*nDim;
    
    for ( stk::mesh::Bucket::size_type k = 0 ; k < length ; ++k ) {
      
      double div_ = 0.0;
      // MJ: dudx has 9 components for every node: 3*3
      for ( int i = 0; i < nDim; ++i ) {
        const int DivOffset = nDim*i;
        div_ += du[k*offSet + DivOffset + i];
      }
