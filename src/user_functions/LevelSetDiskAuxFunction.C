...
LevelSetDiskAuxFunction::LevelSetDiskAuxFunction(const unsigned beginPos,
...
void
LevelSetDiskAuxFunction::do_evaluate(
  const double *coords,
  const double /*time*/,
...
//  const double eps = 2.5 * 0.1667;		//FIX ME!!
  for(unsigned p=0; p < numPoints; ++p) {
    ...
    double phiCircle = -(r-radius_); //THIS IS THE ORIGINAL
