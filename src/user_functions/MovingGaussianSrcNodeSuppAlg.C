...
// Class Definition
//==========================================================================
// MovingGaussianSrcNodeSuppAlg - base class for algorithm
//==========================================================================
//--------------------------------------------------------------------------
//-------- constructor -----------------------------------------------------
//--------------------------------------------------------------------------
MovingGaussianSrcNodeSuppAlg::MovingGaussianSrcNodeSuppAlg(
  Realm &realm,
  double rho0,
  double rho1,
  double cp0, 
...
//--------------------------------------------------------------------------
void
MovingGaussianSrcNodeSuppAlg::setup()
{
...
//--------------------------------------------------------------------------
void
MovingGaussianSrcNodeSuppAlg::node_execute(
  double *lhs,
  double *rhs,
...
//                        (z - zm_) * (z - zm_)  );
  ...
  if (stateCurr_ == 1)
  {
    Qlaser_ = 2 * laserFactor * beamPower_/(M_PI * beamRadius_ * beamRadius_);
  }
  else 
  {
    Qlaser_ = 0.0;
...
/*  double ze = zm_;
  double zi = realm_.solutionOptions_->keyholeDepth_;
  double re = beamRadius_;
  double ri = re/2.0;
  ...
  long double qflux = laserFactor * beamPower_ * beamEff_ / (M_PI * (1.0 - std::exp(-3.0) )* (E + F)) *
                 ( (1.0 - chi)/(ze - zi) * z + (chi * ze - zi)/(ze - zi) ) * 
                 std::exp( c );*/

  double c = - laserFactor * r2 / (beamRadius_ * beamRadius_);
  rhs[0] +=  beamEff_ * Qlaser_ * (  exp(c)  )*dualVolume * std::abs(dIdx[2]) ;
//             ( 2.0 * cp * rho/( cp0_*rho0_ + cp1_*rho1_ ) );
  realm_.sumFreeFlux_ +=  beamEff_ * Qlaser_ * (  exp(c)  )*dualVolume * std::abs(dIdx[2]) ;
//             ( 2.0 * cp * rho/( cp0_*rho0_ + cp1_*rho1_ ) );
  lhs[0] += 0.0;
}
