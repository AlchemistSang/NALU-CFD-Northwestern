  ...
  if (y_material_propertys) {
    ...
    if (y_refs) {
      for (size_t ispec = 0; ispec < y_refs->size(); ++ispec) {
        ...
        get_if_present(y_ref, "lookup_data", refData->lookupEnthDataName_);
    ...
    if (y_specs) {
	  ...
	  // set matData
        ...
        // ADDED BY SEL FOR LEVEL SET METHOD
        else if ( thePropType == "heaviside") {
          double primaryVal;
          double secondaryVal;
          get_required(y_spec, "primary_value", primaryVal);
          get_required(y_spec, "secondary_value", secondaryVal);
          matData->type_ = HEAVISIDE_MAT;
          matData->primary_ = primaryVal;
          matData->secondary_ = secondaryVal;
          NaluEnv::self().naluOutputP0() << thePropName
                          << " is a heaviside prop: "
        ...
        else if ( thePropType == "linear") {
          double val0;
          double slope;
          double refVal;
          get_required(y_spec, "offset", val0);
          get_required(y_spec, "slope", slope);
          get_required(y_spec, "reference", refVal);
          matData->type_ = LINEAR_MAT;
          matData->refVal_ = refVal;
          matData->slope_ = slope;
          matData->val0_ = val0;
          NaluEnv::self().naluOutputP0() << thePropName
                          << " is a linear prop:\n "
                          << "\t" << "offset value: "  << val0 << "\n"
                          << "\t" << "slope value: "  << slope << "\n"
                          << "\t" << "reference value: "  << refVal << std::endl;
        }
        else if ( thePropType == "linear_temp_solid") {
          double val0;
          double slope;
          double refVal;
          get_required(y_spec, "offset", val0);
          get_required(y_spec, "slope", slope);
          get_required(y_spec, "reference", refVal);
          matData->type_ = LINEAR_TEMP_SOLID_MAT;
          matData->refVal_ = refVal;
          matData->slope_ = slope;
          matData->val0_ = val0;
          NaluEnv::self().naluOutputP0() << thePropName
                          << " is a linear temperature solidification prop:\n "
                          << "\t" << "offset value: "  << val0 << "\n"
                          << "\t" << "slope value: "  << slope << "\n"
                          << "\t" << "reference value: "  << refVal << std::endl;
        }
        // ADDED BY SEL FOR LEVEL SET METHOD
        else if ( thePropType == "three_phase") {
          double primaryVal;
          double secondaryVal;
          double tertiaryVal;
          get_required(y_spec, "primary_value", primaryVal);
          get_required(y_spec, "secondary_value", secondaryVal);
          get_required(y_spec, "tertiary_value", tertiaryVal);
          matData->type_ = THREE_PHASE_MAT;
          matData->primary_ = primaryVal;
          matData->secondary_ = secondaryVal;
          matData->tertiary_ = tertiaryVal;
          NaluEnv::self().naluOutputP0() << thePropName
                          << " is a three-phase prop: "
                          << primaryVal << " "
                          << secondaryVal << " " 
                          << tertiaryVal << std::endl;
        }
        // ADDED BY ROGER FOR MULTICOMPONENT
        else if (thePropType == "multi_mixture") {
        double primaryVal;
        double secondaryVal;
        double case_Number;
        get_required(y_spec, "primary_value", primaryVal);
        get_required(y_spec, "secondary_value", secondaryVal);
        get_required(y_spec, "mixing_rule", case_Number);
        matData->type_ = MULTI_MIXTURE_MAT;
        matData->primary_ = primaryVal;
        matData->secondary_ = secondaryVal;
        matData->caseNumber_ = case_Number;
        NaluEnv::self().naluOutputP0() << thePropName
            << " is a multi mixture prop: "
            << primaryVal << " "
            << secondaryVal << " "
            << case_Number << std::endl;
        }
        // ADDED BY MJ FOR Questek powderbed simulations
        else if ( thePropType == "powder_bed") {
          double powder;
          double liquid;
          double solid;
          double powder_slope;
          ...
          get_required(y_spec, "powder", powder);
          get_required(y_spec, "liquid", liquid);
          get_required(y_spec, "solid", solid);
          get_required(y_spec, "powder_slope", powder_slope);
          get_required(y_spec, "liquid_slope", liquid_slope);
          get_required(y_spec, "solid_slope", solid_slope);

          matData->type_ = POWDER_BED;
          matData->powderProp_ = powder;
          matData->liquidProp_ = liquid;
          matData->solidProp_ = solid;
          matData->powder_slope_ = powder_slope;
          matData->liquid_slope_ = liquid_slope;
          matData->solid_slope_ = solid_slope;

          NaluEnv::self().naluOutputP0() << thePropName
                          << " is a powder-bed prop: "
                          << powder << " "
                          << liquid << " " 
                          << solid << " "
                          << powder_slope << " " 
                          << liquid_slope << " " 
                          << solid_slope << " " << std::endl;

        }
        // ADDED BY SEL FOR SOLIDIFICATION
        else if ( thePropType == "latent_heat") {
          double liquidusVal;
          double solidusVal;
          double latentVal;
          double refVal;
          get_required(y_spec, "reference_value", refVal);
          get_required(y_spec, "liquidus", liquidusVal);
          get_required(y_spec, "solidus", solidusVal);
          get_required(y_spec, "latent", latentVal);
          matData->type_ = LATENT_MAT;
          matData->liquidus_ = liquidusVal;
          matData->solidus_ = solidusVal;
          matData->latent_ = latentVal;
          matData->refVal_ = refVal;
          NaluEnv::self().naluOutputP0() << thePropName
                          << " is a latent prop: "
                          << refVal << " "
                          << refVal + latentVal/(liquidusVal - solidusVal) << std::endl;
        }
        else {
