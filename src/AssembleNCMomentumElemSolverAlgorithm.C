...
// Class Definition
//==========================================================================
// AssembleNCMomentumElemSolverAlgorithm - add LHS/RHS for uvw momentum
//==========================================================================
//--------------------------------------------------------------------------
//-------- constructor -----------------------------------------------------
//--------------------------------------------------------------------------
AssembleNCMomentumElemSolverAlgorithm::AssembleNCMomentumElemSolverAlgorithm(
  Realm &realm,
  stk::mesh::Part *part,
...
//--------------------------------------------------------------------------
void
AssembleNCMomentumElemSolverAlgorithm::initialize_connectivity()
{
...
//--------------------------------------------------------------------------
void
AssembleNCMomentumElemSolverAlgorithm::execute()
{
  ...
  for ( stk::mesh::BucketVector::const_iterator ib = elem_buckets.begin();
    ...
    for ( stk::mesh::Bucket::size_type k = 0 ; k < length ; ++k ) {
      ...
      for ( int ip = 0; ip < numScsIp; ++ip ) {
        ...
        const double rhoL = p_densityNp1[il];
        const double rhoR = p_densityNp1[ir];
        ...
        for ( int i = 0; i < nDim; ++i ) {
          ...
          p_rhs[indexL] -= rhoL * aflux + divUstress;
          p_rhs[indexR] += rhoR * aflux + divUstress;
          ...
          p_lhs[rLiL_i] += alhsfacL * rhoL;
          p_lhs[rRiL_i] -= alhsfacL * rhoR;

          // upwind advection (includes 4th); right node
          const double alhsfacR = 0.5*(tmdot-std::abs(tmdot))*pecfac*alphaUpw
            + 0.5*alpha*om_pecfac*tmdot;
          p_lhs[rRiR_i] -= alhsfacR * rhoR;
          p_lhs[rLiR_i] += alhsfacR * rhoL;
        ...
        for ( int ic = 0; ic < nodesPerElement; ++ic ) {
          ...
          for ( int i = 0; i < nDim; ++i ) {
            ...
            // lhs; il then ir
            p_lhs[rLiC_i] += lhsfacAdv * rhoL;
            p_lhs[rRiC_i] -= lhsfacAdv * rhoR;
...
//--------------------------------------------------------------------------
double
AssembleNCMomentumElemSolverAlgorithm::van_leer(
  const double &dqm,
  const double &dqp,
  const double &small)
{
